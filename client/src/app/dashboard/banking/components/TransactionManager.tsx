...
export default function TransactionManager({ transactions }: TransactionManagerProps) {
  ...
  const categories = ['all', 'Office Supplies', 'Transportation', 'Software', 'Client Payments', 'Utilities', 'Marketing'];
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="flex justify-between items-center text-sm text-gray-600">
            ...
            <div className="flex gap-4">
              <span>Total: ${filteredTransactions.reduce((sum, t) => sum + t.amount, 0).toFixed(2)}</span>
              <span>Income: ${filteredTransactions.filter(t => t.amount > 0).reduce((sum, t) => sum + t.amount, 0).toFixed(2)}</span>
              <span>Expenses: ${Math.abs(filteredTransactions.filter(t => t.amount < 0).reduce((sum, t) => sum + t.amount, 0)).toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
