...
export default function NewMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Portal>
      ...
      <div ref={ref} className="absolute right-0 top-12 z-[95] w-[900px] max-w-[95vw] overflow-hidden rounded-xl border bg-white shadow-2xl">
        <div className="flex gap-6 p-4 overflow-x-auto">
          <Section title="GENERAL" items={['Add Users','Item','Inventory Adjustments','Journal Entry','Log Time','Weekly Log']} />
          <Section title="SALES" items={['Customer','Quotes','Delivery Challan','Invoices','Recurring Invoice','Retail Invoice','Sales Order','Customer Payment','Credit Notes']} />
          <Section title="PURCHASES" items={['Vendor','Expenses','Recurring Expense','Bills','Recurring Bills','Purchase Orders','Vendor Payment','Vendor Credits']} />
