...
interface ImportBillsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportSuccess: (bills: any[]) => void;
}

export default function ImportBillsModal({ isOpen, onClose, onImportSuccess }: ImportBillsModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportResults({
        success: true,
        message: `Successfully imported ${result.length} bills`,
        data: result,
        errors: []
      });
  ...
  const downloadTemplate = () => {
    const templateData = [
      ['Vendor Name', 'Bill Date', 'Due Date', 'Amount', 'Currency', 'Notes'],
      ['ABC Corporation', '2024-01-15', '2024-02-15', '1000.00', 'INR', 'Office supplies'],
      ['XYZ Suppliers', '2024-01-16', '2024-02-16', '2500.00', 'INR', 'Equipment purchase']
    ];
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Import Bills</h3>
          <button
            onClick={handleClose}
            ...
            <>
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Upload a CSV file with your bills data. Make sure the file follows the correct format.
                </p>
              </div>
              ...
              {importResults.success && importResults.data.length > 0 && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                  <p className="text-sm text-green-800">
                    Successfully imported {importResults.data.length} bills.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex space-x-3">
            {!importResults ? (
              <>
                ...
                >
                  {isUploading ? 'Importing...' : 'Import Bills'}
                </button>
              </>
            ) : (
              <button
