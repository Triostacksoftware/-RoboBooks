...
const EditCreditNotePage = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setCustomerName(mockCreditNote.customerName);
      setCreditNoteNumber(mockCreditNote.creditNoteNumber);
      setReferenceNumber(mockCreditNote.referenceNumber);
      setCreditNoteDate(mockCreditNote.creditNoteDate);
      setSalesperson(mockCreditNote.salesperson);
      setSubject(mockCreditNote.subject);
      setItems(mockCreditNote.items);
      setSubTotal(mockCreditNote.subTotal);
      setDiscount(mockCreditNote.discount);
      setDiscountType(mockCreditNote.discountType);
      setTdsType(mockCreditNote.tdsType);
      setSelectedTax(mockCreditNote.selectedTax);
      setTdsAmount(mockCreditNote.tdsAmount);
      setAdjustment(mockCreditNote.adjustment);
      setTotal(mockCreditNote.total);
      setLoading(false);
    }, 1000);
  }, [params.id]);
  ...
  const handleSubmit = async (status: 'draft' | 'open') => {
    ...
    try {
      ...
      console.log('Updated Credit Note Data:', creditNoteData);
      ...
      router.push(`/dashboard/sales/credit-notes/${params.id}`);
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Modify credit note {creditNoteNumber}</p>
          </div>
        </div>
      </div>
