...
const DeliveryChallanForm: React.FC<DeliveryChallanFormProps> = ({
  ...
  const loadCustomers = useCallback(async () => {
    try {
      // Use actual customer service to load real customers
      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"
        }/api/customers?status=active&limit=1000&sortBy=displayName&sortOrder=asc`,
        {
          credentials: "include",
        }
      );
  ...
  const handleCustomerChange = useCallback(
    (customerId: string, customer?: CustomerType) => {
      setFormData((prev) => ({
        ...prev,
        customerId,
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        formData.items.some(
          (item) => !item.itemName || !item.quantity || !item.uom
        )
      ) {
      ...
      if (
        ...
        Math.abs(formData.adjustment) > formData.subTotal
      ) {
      ...
      if (
        ...
        !formData.shipTo.address ||
        !formData.shipTo.city ||
      ...
      if (
        !formData.dispatchFrom.name ||
        !formData.dispatchFrom.address ||
        !formData.dispatchFrom.city ||
        !formData.dispatchFrom.state
      ) {
      ...
      try {
        ...
        if (
          ...
          authResponse !== null &&
          "success" in authResponse &&
          (authResponse as any).success
        ) {
      ...
      try {
        nextNumberResponse = await deliveryChallanService.getNextChallanNumber(
          userId, // Use actual user ID instead of hardcoded string
          fy,
          "DC"
        );
      } catch (error) {
      ...
      if (
        nextNumberResponse.success &&
        nextNumberResponse.data?.nextChallanNumber
      ) {
        // Create items first if they don't exist, then map them
        const mappedItems = [];
        for (const item of formData.items) {
          try {
            ...
            const itemResponse = await fetch(
              `${
                process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"
              }/api/items/search?q=${encodeURIComponent(item.itemName)}`,
              {
                credentials: "include",
              }
            );
            ...
            toast.error(`Failed to process item: ${item.itemName}`);
            return;
          }
        }
        ...
        const challanData = {
          ...
          items: mappedItems,
          challanNo:
            formData.challanId || nextNumberResponse.data.nextChallanNumber,
          fy,
          numberingSeries: "DC",
          orgId: userId, // Use the user ID as orgId since it references Admin
        };

        let response;
        try {
          if (mode === "edit") {
            response = await deliveryChallanService.update(
              initialData._id,
              challanData
            );
          } else {
          ...
          toast.error(
            `Failed to ${action} delivery challan. Please try again.`
          );
          return;
        }
            ...
            <div className="flex items-center space-x-2">
              <div className="flex-1">
                <input
                  ...
                  value={
                    formData.challanId
                      ? formatChallanIdForDisplay(formData.challanId)
                      : ""
                  }
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    handleAddressChange("shipTo", "address", e.target.value)
                  }
                  rows={2}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm hover:border-gray-300 transition-all duration-200"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleAddressChange("shipTo", "zipCode", e.target.value)
                    }
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm hover:border-gray-300 transition-all duration-200"
                  />
                </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleAddressChange("dispatchFrom", "name", e.target.value)
                  }
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm hover:border-gray-300 transition-all duration-200"
                  required
                />
