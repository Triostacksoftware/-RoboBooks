...
export default function PaymentDetailsPanel({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        paymentHistoryDropdownRef.current &&
        !paymentHistoryDropdownRef.current.contains(event.target as Node)
      ) {
        setShowPaymentHistoryDropdown(false);
      }
      if (
        uploadFilesDropdownRef.current &&
        !uploadFilesDropdownRef.current.contains(event.target as Node)
      ) {
  ...
  const getAmountInWords = (amount: number): string => {
    ...
    if (amount < 1000)
      return (
        ...
        " Hundred" +
        (amount % 100 > 0 ? " and " + getAmountInWords(amount % 100) : "")
      );
    if (amount < 100000)
      return (
        getAmountInWords(Math.floor(amount / 1000)) +
        " Thousand" +
        (amount % 1000 > 0 ? " " + getAmountInWords(amount % 1000) : "")
      );
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="relative" ref={uploadFilesDropdownRef}>
            <button
              onClick={() =>
                setShowUploadFilesDropdown(!showUploadFilesDropdown)
              }
