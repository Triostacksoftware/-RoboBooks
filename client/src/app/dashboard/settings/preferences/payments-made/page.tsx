...
const columnOptions = [
  { key: 'paymentNumber', label: 'Payment Number', required: true },
...
export default function PaymentsPreferencesPage() {
  const router = useRouter();
  const [preferences, setPreferences] = useState<PaymentPreferences>(defaultPreferences);
  ...
  const loadPreferences = async () => {
    try {
      setLoading(true);
      const savedPreferences = await preferencesService.getPreferences('payments-made');
      if (savedPreferences) {
  ...
  const savePreferences = async () => {
    try {
      setSaving(true);
      setMessage(null);
      await preferencesService.savePreferences('payments-made', preferences);
  ...
  const resetToDefaults = async () => {
    try {
      setLoading(true);
      const resetPrefs = await preferencesService.resetPreferences('payments-made');
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Payments Made Preferences</h1>
                <p className="text-gray-600">Customize how payments are displayed and managed</p>
              </div>
            </div>
