...
interface RecurringBillPreferences {
...
const defaultPreferences: RecurringBillPreferences = {
...
const columnOptions = [
  ...
  { key: 'nextDueDate', label: 'Next Due Date', required: true },
  { key: 'status', label: 'Status', required: true },
  { key: 'description', label: 'Description', required: false },
  { key: 'createdAt', label: 'Created Date', required: false },
];

export default function RecurringBillsPreferencesPage() {
  const router = useRouter();
  const [preferences, setPreferences] = useState<RecurringBillPreferences>(defaultPreferences);
  ...
  const loadPreferences = async () => {
    try {
      setLoading(true);
      const savedPreferences = await preferencesService.getPreferences('recurring-bills');
      if (savedPreferences) {
  ...
  const savePreferences = async () => {
    try {
      setSaving(true);
      setMessage(null);
      await preferencesService.savePreferences('recurring-bills', preferences);
  ...
  const resetToDefaults = async () => {
    try {
      setLoading(true);
      const resetPrefs = await preferencesService.resetPreferences('recurring-bills');
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Recurring Bills Preferences</h1>
                <p className="text-gray-600">Customize how recurring bills are displayed and managed</p>
              </div>
            </div>
