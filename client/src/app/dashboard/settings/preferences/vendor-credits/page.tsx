...
interface VendorCreditPreferences {
...
const defaultPreferences: VendorCreditPreferences = {
...
const sortOptions = [
  { value: 'creditNumber', label: 'Credit Number' },
  { value: 'vendorName', label: 'Vendor Name' },
  { value: 'creditDate', label: 'Credit Date' },
...
const columnOptions = [
  { key: 'creditNumber', label: 'Credit Number', required: true },
  { key: 'vendorName', label: 'Vendor Name', required: true },
  { key: 'creditDate', label: 'Credit Date', required: true },
...
export default function VendorCreditsPreferencesPage() {
  const router = useRouter();
  const [preferences, setPreferences] = useState<VendorCreditPreferences>(defaultPreferences);
  ...
  const loadPreferences = async () => {
    try {
      setLoading(true);
      const savedPreferences = await preferencesService.getPreferences('vendor-credits');
      if (savedPreferences) {
  ...
  const savePreferences = async () => {
    try {
      setSaving(true);
      setMessage(null);
      await preferencesService.savePreferences('vendor-credits', preferences);
  ...
  const resetToDefaults = async () => {
    try {
      setLoading(true);
      const resetPrefs = await preferencesService.resetPreferences('vendor-credits');
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Vendor Credits Preferences</h1>
                <p className="text-gray-600">Customize how vendor credits are displayed and managed</p>
              </div>
            </div>
