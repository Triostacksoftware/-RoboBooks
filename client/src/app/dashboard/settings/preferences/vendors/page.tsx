...
const defaultPreferences: VendorPreferences = {
...
const sortOptions = [
  ...
  { value: 'unusedCredits', label: 'Unused Credits' },
...
const columnOptions = [
  ...
  { key: 'payables', label: 'Payables', required: true },
  { key: 'unusedCredits', label: 'Unused Credits', required: false },
...
export default function VendorsPreferencesPage() {
  const router = useRouter();
  const [preferences, setPreferences] = useState<VendorPreferences>(defaultPreferences);
  ...
  const loadPreferences = async () => {
    try {
      setLoading(true);
      const savedPreferences = await preferencesService.getPreferences('vendors');
      if (savedPreferences) {
  ...
  const savePreferences = async () => {
    try {
      setSaving(true);
      setMessage(null);
      await preferencesService.savePreferences('vendors', preferences);
  ...
  const resetToDefaults = async () => {
    try {
      setLoading(true);
      const resetPrefs = await preferencesService.resetPreferences('vendors');
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Vendors Preferences</h1>
                <p className="text-gray-600">Customize how vendors are displayed and managed</p>
              </div>
            </div>
