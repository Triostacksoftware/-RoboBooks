...
const ALL_PHONE_OPTIONS: CountryDialOption[] = [
  ...
  // You can add micro-territories here as needed.
];
/* ------------------------------------------------- */
...
function AutoScaledFrame({
  ...
  return (
    <div style={{ width: reservedW, height: reservedH }} className="pointer-events-none md:pointer-events-auto">
      <div style={{ width: targetW, height: targetH, transform: `scale(${scale})`, transformOrigin: "center center" }}>
        {children}
      </div>
    </div>
  );
...
export default function Register() {
  ...
  const defaultIN = useMemo(
    () => ALL_PHONE_OPTIONS.find((o: CountryDialOption) => o.iso2 === "IN" && o.dial === "+91"),
    []
  );
  ...
  return (
    <main className="relative min-h-[100svh] bg-gradient-to-b from-sky-50 to-emerald-50">
      <div className="mx-auto flex min-h-[100svh] max-w-7xl items-center justify-center px-4 md:px-6">
        <AutoScaledFrame targetW={1160} targetH={760} marginX={48} marginY={96} enableAtMinWidth={768}>
          {/* === CARD === */}
          {/* Remove strict clipping; allow inner columns to scroll if needed */}
          <section className="relative grid h-full w-full rounded-3xl bg-white/70 backdrop-blur-xl shadow-2xl ring-1 ring-black/5 md:grid-cols-2">
            ...
            <section className="hidden w-full flex-col justify-between bg-blue-700 p-8 text-white lg:flex">
              <div className="relative">
                <h2 className="text-4xl font-bold leading-tight">Trusted by <br /> businesses and CAs</h2>

                <article className="mt-10 rounded-2xl bg-blue-800/70 p-6">
                  <div className="text-6xl leading-none opacity-80">“</div>
                  <p className="mt-4 text-lg leading-8">
                    Running a business requires smart accounts management. Robo Books simplifies everything from invoicing to inventory in one platform.
                  </p>

                  <div className="mt-6 flex items-center gap-3">
                    <div className="grid h-10 w-10 place-items-center rounded-full bg-yellow-300 text-blue-900 font-bold">N</div>
                    <div>
                      <p className="font-semibold">Naveedh V.V</p>
                      <p className="text-xs text-blue-100">CO-FOUNDER, MAKE YOUR OWN PERFUME</p>
                    </div>
                  </div>
                ...
                <div className="mt-10 text-center text-xs tracking-widest text-white/80">RATED BY THE BEST</div>
                <div className="mt-4 grid grid-cols-4 gap-4">
                  <div className="rounded-xl bg-blue-800/50 px-3 py-2 text-center text-sm">Capterra ★ 4.4/5</div>
                  <div className="rounded-xl bg-blue-800/50 px-3 py-2 text-center text-sm">G2 ★ 4.4/5</div>
                  <div className="rounded-xl bg-blue-800/50 px-3 py-2 text-center text-sm">Play ★ 4.7/5</div>
                  <div className="rounded-xl bg-blue-800/50 px-3 py-2 text-center text-sm">App Store ★ 4.8/5</div>
                </div>
              </div>
            </section>

            {/* RIGHT: form (make this column scrollable if content overflows) */}
            <div className="relative flex max-h-full min-h-0 flex-col overflow-y-auto p-6 md:p-8 lg:p-10">
              {/* Brand */}
              ...
              <h1 className="mt-4 text-[26px] md:text-[28px] font-semibold leading-7 tracking-tight">Create your account</h1>
              ...
              <form onSubmit={onSubmit} className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-1">
                  <CountryDialSelector
                    ...
                    onChange={(opt: CountryDialOption) =>
                      setForm((prev) => ({ ...prev, phoneIso2: opt.iso2, phoneDialCode: opt.dial }))
                    }
                  />
                ...
                {/* State (right) */}
                <label className="block">
                  <span className="mb-1.5 block text-sm font-medium text-slate-700">State</span>
                  <select
                    id="state"
                    value={form.state}
                ...
                <label className="md:col-span-2 flex items-start gap-3 text-sm text-slate-700">
                  ...
                  <span>
                    ...
                    and{" "}
                    <Link href="/legal/privacy" className="text-blue-700 hover:underline">Privacy Policy</Link>.
                  </span>
                </label>
              ...
              {/* Social sign up — icons only (now always visible; pane can scroll if needed) */}
              <div className="mt-6">
                <p className="text-sm text-slate-500">Or sign up using</p>
                <div className="mt-3 flex min-h-[52px] flex-wrap items-center gap-3">
                  <ProviderButton label="Apple" onClick={() => (window.location.href = "/api/auth/signin/apple")}>
                    <AppleMark className="h-5 w-5 text-black" />
                  </ProviderButton>
                  <ProviderButton label="Google" onClick={() => (window.location.href = "/api/auth/signin/google")}>
                    <GoogleMark className="h-5 w-5" />
                  </ProviderButton>
                  <ProviderButton label="LinkedIn" onClick={() => (window.location.href = "/api/auth/signin/linkedin")}>
                    <LinkedInMark className="h-5 w-5 text-[#0A66C2]" />
                  </ProviderButton>
                  <ProviderButton label="GitHub" onClick={() => (window.location.href = "/api/auth/signin/github")}>
                    <GitHubMark className="h-5 w-5 text-black" />
                  </ProviderButton>
                  <ProviderButton label="Microsoft" onClick={() => (window.location.href = "/api/auth/signin/azure-ad")}>
              ...
              <p className="mt-6 text-sm text-slate-600">
                ...
                <a className="font-semibold text-blue-700 hover:underline" href="/signin">Sign in</a>
              </p>
            </div>
          </section>
          {/* === end card === */}
